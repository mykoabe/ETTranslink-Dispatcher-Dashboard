...
export default function NewForm({ inputs }) {
  ...
  const handleFacilitator = async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db, "facilitators", res.user.uid), {
        ...data,
  ...
  return (
    <>
      ...
      {open && (
        <DialogBox
          open={open}
          title={"New Facilitator is created"}
          content={
            "New facilitator is just created you can send the credentials and the facilitaor can use these credentials to login"
          }
